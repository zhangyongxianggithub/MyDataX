...
import com.alibaba.datax.plugin.writer.httpwriter.converter.ObjectRecordConverter;
import com.alibaba.datax.plugin.writer.httpwriter.converter.TypeHandlerRegistry;
import com.alibaba.datax.plugin.writer.httpwriter.validator.ConfigurationValidator;
...
import static com.alibaba.datax.plugin.writer.httpwriter.Key.FAIL_FAST;
import static com.alibaba.datax.plugin.writer.httpwriter.Key.FIELDS;
...
import static com.alibaba.datax.plugin.writer.httpwriter.Key.PRINT;
 ...
 */
@Slf4j
public class HttpJsonWriter extends Writer {
    ...
    public static class Task extends Writer.Task {
        ...
        public void startWrite(final RecordReceiver lineReceiver) {
            ...
            if (batchMode && !writerBuffer.isEmpty()) {
                this.doWrite(writerBuffer);
                writerBuffer.clear();
            }
        ...
        public void post() {
            ...
            log.info(
                    ...
                    this.getPluginName(), taskIndex,
                    Instant.ofEpochMilli(startTime)
                            .atZone(ZoneId.systemDefault()).toLocalDateTime(),
                    Instant.ofEpochMilli(endTime).atZone(ZoneId.systemDefault())
                            .toLocalDateTime(),
                    Duration.ofMillis(endTime - startTime), successCount);
            if (failCount > 0) {
                log.error("job {} task {} execute to end, fail count: {}",
                        this.getPluginName(), taskIndex, failCount);
            }
        }
